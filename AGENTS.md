...
`rig/` is the agent-neutral view of the same kit Claude Code reads from
`.claude/`: the skill files are flattened copies, and `agents/`, `scripts/`,
`docs/`, `REVIEWER.md`, and `label-mapping.md` are symlinks into `.claude/`,
so there is one source of truth per file.
<!-- rig:end -->
