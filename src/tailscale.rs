...
mod tests {
    ...
    mod wants_tailnet {
        ...
        /// The opt-in model: a join needs explicit intent from `--tailnet`,
        /// `--tailnet-tag`, or `[tailscale] auto = true`, and each source opts
        /// in on its own. An auth key sitting in the environment is
        /// deliberately not consulted.
        #[test]
        fn needs_explicit_intent() {
    ...
    mod resolve_tag {
        ...
        /// An explicit `--tailnet-tag` outranks both config and the default.
        #[test]
        fn flag_wins() {
            assert_eq!(
        ...
        /// With no flag, `[tailscale] tag` outranks the default.
        #[test]
        fn config_wins() {
            assert_eq!(
    ...
    mod validate_tag {
        ...
        /// A well-formed ACL tag is `tag:` followed by a non-empty name.
        #[test]
        fn accepts_prefixed() {
        ...
        /// A bare name without the `tag:` prefix is not a tag.
        #[test]
        ...
        /// `tag:` with nothing after it names no tag.
        #[test]
    ...
    mod require_auth_key {
        use super::*;

        /// The error names the env var that was expected to hold the key, so
        ...
        /// An env var set to the empty string is not a usable credential.
        #[test]
        fn errs_when_empty() {
    ...
    mod tailscale_up_command {
        use super::*;

        /// The generated script advertises the tag, enables SSH, and passes the
        /// auth key by env-var reference -- never inline, so the key can't leak
        /// into a process listing. Ordering matters too: `tailscale up` has to
        /// hard-fail before `serve` runs, while `serve` itself stays
    ...
    mod node_hostname {
        ...
        /// `tailscale status --json` reports the FQDN with a trailing dot.
        #[test]
        ...
        /// The guest command can emit banner lines ahead of the JSON, so
        /// parsing starts at the first `{` rather than at byte zero.
        #[test]
    ...
    mod is_valid_env_var_name {
        ...
        /// Anything outside `[A-Za-z_][A-Za-z0-9_]*` is rejected -- including a
        /// name carrying shell metacharacters, since these names are
        /// interpolated into a command that runs in the guest.
        #[test]
        fn rejects_bad() {
    ...
    mod box_display_url {
        ...
        /// A MagicDNS name is the better URL when the box has one.
        #[test]
        fn prefers_magicdns() {
            assert_eq!(
    ...
    mod tailscale_ssh_args {
        use super::*;

        /// No command -> an interactive session.
        #[test]
        fn builds_interactive_argv() {
            assert_eq!(
    ...
    mod resolve_join_mode {
        ...
        /// An OAuth client id plus a resolvable secret mints an ephemeral key.
        #[test]
        fn mints_with_oauth() {
        ...
        /// `oauthClientSecretEnv` overrides which env var holds the secret.
        #[test]
        fn uses_custom_secret_env() {
        ...
        /// Only a static auth-key env var resolves -> use it directly.
        #[test]
        fn uses_static_env() {
        ...
        /// No credential resolves -> skip the join rather than fail the boot.
        #[test]
        ...
        /// With both available, minting wins -- an ephemeral key is preferred
        /// over a long-lived static one.
        #[test]
        fn prefers_oauth() {
        ...
        /// An OAuth secret that resolves to the empty string is not a usable
        /// credential, so fall through to the static env var instead of
        /// attempting a mint that would fail.
        #[test]
    ...
    mod create_key_body {
        ...
        /// The minted key must be single-use, ephemeral, and preauthorized, and
        /// must carry the requested tag -- the shape the Tailscale API expects.
        #[test]
    ...
    mod oauth_token_form {
        use super::*;

        /// The client-credentials form carries `client_id`, `client_secret`, and
        /// a `grant_type` of `client_credentials`.
        #[test]
    ...
    mod join_failure_detail {
        use super::*;

        /// Exit 3 means the image ships no tailscaled: point at `lilbox-box`
        /// and name the image that fell short.
        #[test]
        fn suggests_lilbox_box() {
            let detail = join_failure_detail(3, "", "python");
            assert!(detail.contains("lilbox-box"));
            assert!(detail.contains("python"));
        }

        /// Exit 4 means tailscaled is present but didn't come up.
        #[test]
        fn names_tailscaled() {
        ...
        /// Real stderr from the guest beats any canned message, for every code.
        #[test]
        fn prefers_stderr() {
            for code in [3, 4, 5, 1] {
                assert_eq!(
